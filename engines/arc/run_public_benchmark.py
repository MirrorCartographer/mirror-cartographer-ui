"""
Run the ARC benchmark harness with the current blinded dual-track solver.

Voice-readable behavior:
Give this script a folder of ARC JSON tasks. It runs the current solver on each task, checks attempt one and attempt two against the public answer, and writes a result JSON. This is the file that turns solver claims into proof artifacts.
...
from engines.arc.blinded_dual_track_solver_v8 import solve_task_blinded_v8
...
def main() -> None:
    ...
    result = run_benchmark(args.data_dir, args.out, solve_task_blinded_v8)
