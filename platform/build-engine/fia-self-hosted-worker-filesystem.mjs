...
export function buildMountPlan({source,packageCache,outputStage,tempDir,runtimePaths=[]}){
 ...
 for(const [i,p] of runtimePaths.entries())mounts.push({source:safeAbs(p,`runtime path ${i}`),target:validateGuestPath(`/runtime/${i}`),mode:'ro',kind:'bind'});
...
function probeProduction({cgroupRoot}){
 ...
 for(const p of ['/usr/bin/unshare','/usr/bin/mount','/usr/sbin/chroot'])if(spawnSync('/usr/bin/test',['-x',p]).status!==0)return {ok:false,reason:`missing executable ${p}`};
...
function sandboxScript({root,mounts,nodeGuest,command}){
 ...
 for(const m of mounts){const dst=path.join(root,m.target);lines.push(`mkdir -p ${shellQuote(dst)}`,`mount --bind ${shellQuote(m.source)} ${shellQuote(dst)}`);if(m.mode==='ro')lines.push(`mount -o remount,bind,ro,nosuid,nodev ${shellQuote(dst)}`);else lines.push(`mount -o remount,bind,rw,nosuid,nodev ${shellQuote(dst)}`)}
...
export async function executeFilesystemWorker({source,packageCache,output,command,nodeBinary=process.execPath,nodeBinarySha256,runtimePaths=[],cgroupRoot='/sys/fs/cgroup',limits={},timeoutMs=180000,backend='linux-minimal',testOnly=false}){
 ...
 try{await fs.mkdir(stage,{recursive:true});const mounts=buildMountPlan({source,packageCache,outputStage:stage,tempDir:temp,runtimePaths:[nodeBinary,...runtimePaths]});const policy=buildCgroupPolicy(limits);let isolation;
  ...
  if(backend==='linux-minimal'){const script=sandboxScript({root,mounts,nodeGuest:'/runtime/0',command});argv=['/usr/bin/unshare','--user','--map-root-user','--net','--mount','--pid','--fork','/bin/sh','-c',script]}else argv=command;
