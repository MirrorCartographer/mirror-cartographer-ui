...
export async function executeAccountedWorker({cgroupDir,command,outputDir,evidencePath,env={},timeoutMs=180000,verifyOutput,simulateCgroup=false}){
 ...
 const outputBefore=hash(Buffer.from(canon(await inventory(outputDir))));let verification;try{verification=await verifyOutput({outputDir,result,outcome,outputIdentity:outputBefore});if(!verification||verification.ok!==true)throw Error('output verification failed');const outputAfter=hash(Buffer.from(canon(await inventory(outputDir))));if(outputAfter!==outputBefore)throw Error('output mutated during verification');verification={...verification,outputIdentity:outputBefore}}finally{await setFreeze(cgroupDir,0,simulateCgroup);if(await exists(path.join(cgroupDir,'cgroup.freeze')))await waitFrozen(cgroupDir,0)}
