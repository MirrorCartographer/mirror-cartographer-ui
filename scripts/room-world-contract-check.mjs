...
assert.equal(ROOM_COUNT, 101, 'room count must remain 101');
assert.equal(ROOMS.length, 101, 'catalog must contain 101 rooms');
assert.equal(LAYERS_PER_ROOM, 20, 'each room must expose 20 layers');
assert.equal(new Set(ROOMS.map((room) => room.id)).size, 101, 'room ids must be unique');
...
assert.ok(new Set(signatures).size >= 75, 'catalog must maintain strong room-level uniqueness');
