...
function drawVisualScore(ctx, env) {
  ...
  const phrasePhase = clamp01(clock?.phrasePhase ?? phase);
  const density = clamp01(clock?.density ?? 0.5);
  const phraseDrift = Number.isFinite(clock?.phrase) ? Math.sin(clock.phrase * 0.65) : 0;
  const breath = 0.72 + density * 0.56;
  const baseY = h * (0.78 + phraseDrift * 0.012 - Math.sin(phrasePhase * TAU) * 0.01 * density);
  const span = Math.min(w * (0.66 + density * 0.1), 420);
  ...
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  ctx.strokeStyle = colorFor(state, 0.09 + pulse * 0.14 + density * 0.08);
  ctx.lineWidth = 0.8 + density * 0.35;
  for (let line = -1; line <= 1; line += 1) {
    const lineBreath = Math.sin(phrasePhase * TAU + line * 0.7) * density * 3;
    ctx.beginPath();
    ctx.moveTo(startX - 8, baseY + line * (11 + density * 3) + lineBreath);
    ctx.lineTo(startX + span + 8, baseY + line * (11 + density * 3) - lineBreath);
    ctx.stroke();
  }
  score.forEach((mark, index) => {
    const age = Math.min(1, (Date.now() - mark.time) / 7200);
    const life = 1 - age;
    const phrase = Math.sin(mark.x * 5 + mark.y * 7 + mark.spin + beat * 0.03 + phrasePhase * TAU);
    const clockLift = Math.sin((phrasePhase + index / Math.max(1, score.length)) * TAU) * (2.2 + rhythm * 0.08 + density * 7);
    const densitySpread = (index - (score.length - 1) / 2) * density * 1.9;
    const x = startX + index * stepX + densitySpread;
    const y = baseY - phrase * (24 + density * 16) - rhythm * 1.05 - clockLift;
    const size = 3.2 + life * 5.4 + pulse * 4.4 + density * 2.4 + Math.max(0, clockLift) * 0.1;
    ctx.globalAlpha = Math.min(0.82, 0.12 + life * 0.34 + density * 0.2);
    ctx.strokeStyle = colorFor(mark.kind, 0.3 + life * 0.32 + density * 0.16);
    ctx.fillStyle = colorFor(mark.kind, 0.04 + life * 0.08 + density * 0.05);
    ctx.shadowColor = colorFor(mark.kind, 0.9);
    ctx.shadowBlur = 6 + life * 10 + density * 10;
    ctx.beginPath();
    ctx.ellipse(x, y, size * (1.35 + density * 0.22), size * breath, Math.sin(t * 0.012 + index + phase + phrasePhase) * 0.35, 0, TAU);
    ctx.fill();
    ctx.stroke();
  });
  ctx.restore();
