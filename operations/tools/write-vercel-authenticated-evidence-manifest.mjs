...
export function validateAuthenticatedEvidenceManifest(input) {
  ...
  if (!boundary || boundary.deployment_identity !== false || boundary.browser_audibility !== false || boundary.physical_device_behavior !== false) {
    return fail('claim_boundary_invalid');
  }
...
export async function writeAuthenticatedEvidenceManifest({ input_path, output_path }) {
  ...
  const manifest = {
    ...
    validation,
    limitations: [
      'This manifest proves only the retained authenticated workflow enumeration described by its inputs.',
      'It does not prove Vercel deployment identity, browser audibility, or physical-device behavior.'
    ]
  };
  try {
    await writeFile(output_path, `${JSON.stringify(manifest, null, 2)}\n`, { flag: 'wx' });
  } catch (error) {
    return fail(error.code === 'EEXIST' ? 'output_exists' : 'output_write_failed', { code: error.code ?? 'unknown' });
  }
