...
test('accepts retained curl capability and hostname evidence while preserving runtime activation false', () => {
...
test('rejects unsupported curl before hostname publication assessment', () => {
  ...
  assert.equal(result.violations.includes('curl_pipeline:capability:curl_version:method_metric_unavailable_before_7_72_0'), true);
  assert.equal(result.evidence.curl_receipt_sha256, null);
});
...
test('rejects redirect-following curl command after capability preflight', () => {
  ...
  assert.equal(result.curl_pipeline_verified, false);
  assert.equal(result.curl_receipt_verified, false);
  assert.equal(result.violations.includes('curl_pipeline:hostname:command:redirect_following_forbidden'), true);
});
...
test('rejects TLS verification failure', () => {
  ...
  const result = assessCurlBoundRepertoryPublicationReadiness(input);
  assert.equal(result.ready, false);
  assert.equal(result.violations.includes('curl_pipeline:hostname:curl:tls_verification_failed'), true);
});

test('rejects deployment commit mismatch before curl capability or hostname adaptation', () => {
  const input = validInput();
...
test('rejects repertory runtime boundary weakening after verified curl capability and hostname evidence', () => {
  const input = validInput();
