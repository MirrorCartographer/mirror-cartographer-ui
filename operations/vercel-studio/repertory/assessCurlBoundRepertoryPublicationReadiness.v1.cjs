...
const { adaptCurlGeneratedHostnameReceipt } = require('../../frontier-research/curlGeneratedHostnameReceipt.v1.cjs');
...
function assessCurlBoundRepertoryPublicationReadiness(input) {
  ...
  const curl = adaptCurlGeneratedHostnameReceipt({
  ...
  if (!curl.verified) {
    return {
      ...
      expected_commit_sha: expectedCommitSha || null,
      curl_receipt_verified: false,
      violations: curl.violations.map((v) => `curl:${v}`),
      claim_boundary: 'publication_prohibited',
      evidence: {
  ...
  return {
    ...
    claim_boundary: publication.ready && violations.length === 0
      ? 'publication_preconditions_verified_from_retained_curl_receipt_only_runtime_activation_not_performed'
