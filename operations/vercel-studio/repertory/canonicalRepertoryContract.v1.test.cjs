...
test('canonical repertory satisfies durable provenance and deterministic scheduling contracts', () => {
  ...
  assert.equal(repertory.selection_rule.type, 'deterministic_modulo');
  ...
  for (const slot of repertory.hour_slots) {
    ...
    assert.equal(
      slot.production_id,
      repertory.productions[slot.utc_hour % repertory.productions.length].id,
      `non-deterministic production mapping for UTC hour ${slot.utc_hour}`,
    );
  }
